...
export async function placeBid(eventId: string, amount: number, userId: string) {
...
export async function getBids(eventId: string) {
    ...
    try {
      ...
      if (usersError) throw new Error(usersError.message)
        ...
        // 4. Map and format final bid list
  ...
  export async function getAllLatestBidsOfUser(userId: string) {
    ...
    try {
      ...
      const { data: userBids, error: bidsError } = await supabase
        ...
        `)
        .eq("user_id", userId)
        .order("created_at", { ascending: false }) as { data: BidWithEvent[] | null, error: { message: string } | null }
